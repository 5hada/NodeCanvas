import type { ConnectionValidationResult, NodeCanvasAdapter } from '../../core'
import type { AppEdgeData, AppGraph, AppNodeData, AppPortData } from '../domain'
  ...
  return {
    validateConnection: (context) => {
      const results = extensions
        ...
        .filter((result): result is ConnectionValidationResult => Boolean(result))
    ...
    validateGraph: (graph: AppGraph) =>
      extensions.flatMap((extension) => extension.adapter?.validateGraph?.(graph) ?? []),
  }
}
